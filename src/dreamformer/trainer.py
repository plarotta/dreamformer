...
class Trainer:
    ...
    def _emit_train_progress(self, metrics: dict[str, float]) -> None:
        ...
        message = (
            ...
            f"gate_min={metrics['memory_gate_min']:.4f} "
            f"gate_max={metrics['memory_gate_max']:.4f} "
            f"gate_reg={metrics['memory_gate_reg_loss']:.6f} "
            f"nrem_selected={metrics['nrem_selected']:.0f} "
            f"nrem_rate={metrics['nrem_selection_rate']:.3f} "
            f"stm_raw={metrics['stm_raw_read_norm']:.3f} "
            f"ltm_raw={metrics['ltm_raw_read_norm']:.3f} "
            f"stm_read={metrics['stm_read_norm']:.3f} "
            f"ltm_read={metrics['ltm_read_norm']:.3f} "
            f"stm_scale={metrics['stm_fusion_scale']:.3f} "
            f"ltm_scale={metrics['ltm_fusion_scale']:.3f} "
            f"mix_read={metrics['memory_mix_norm']:.3f} "
